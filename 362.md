...
# Unfair Coupon Tokens Distribution

### Summary

Users can deposit an amount of reserve tokens near the end of the `distributionPeriod` and withdraw right after the distribution occurs. So they can get coupons and claim USDC, but not compromise money in the long term.
Depositing the same amount of tokens 60 days or 1 hour before an Auction starts will generate the same amount of Coupons.

### Root Cause

The calculation logic accounts for the whole period distribution period before the auction, not for the time the user created a position depositing `reserveTokens` on the `Pool` contract.
...
1. The user identifies the time for the next Auction to start.
2. Deposits X amount of `reserveToken` on the pool.
3. Waits for the Auction to finish and coupons to be distributed.
4. Claims its coupons and redeems the ether.

### Impact

Users are not incentivized to maintain money for the long term on the protocol. They can simply wait for the last days or hours and then create positions. This will disrupt the protocol's purposes.
...
```solidity
    ...
    function test_SameRedeemNoMatterCommittingTime() public {
        uint256 resultFromFull = helper_commitMoneyForFullTime();
        console.log("Fulltime :", resultFromFull);
        uint256 resultFromMinimum = helper_commitMoneyForMinimumPeriodOfTime();
        console.log("Small Time:", resultFromMinimum);
        console.log("Difference Between Fulltime & Small Time:", resultFromFull - resultFromMinimum);
    }
...
Consider adding a more restrictive logic on coupon calculations and consider deposits time to do it fairly.