...
# `Auction::bid` allows user to dump bids with the same value

### Summary

The `Auction::bid` function allows users to keep bidding after the limit is reached. The goal is for users with higher bids to take the place of lower bids. However, the bid will always have the same size as the last one, as it is limited by the `totalBuyCouponAmount`. So, a user can send any amount over the last bid, even `1 wei` more than the last bid. It will get its 1 wei refunded, and still dump the previous user of the list allowing users to manipulate valid bids.

### Root Cause

`Auction::removeExcessBids` account for the total amount of the `bid`. Not the real amount that will be deposited (last bid value). So, it basically replaces one bid with another with the same amount and refunds the difference.

https://github.com/sherlock-audit/2024-12-plaza-finance/blob/main/plaza-evm/src/Auction.sol#L298-L331

### Internal Pre-conditions

1. Reach the maximum number of bids
2. Reach the coupon limit

### External Pre-conditions

1. A common user, malicious or not, to call the function sending any amount over the last bid position value.

### Attack Path

1. Checks what is the value of the last bid
2. Bids a slightly greater amount

### Impact

Seize the dumped user's opportunity to buy the reserve tokens without generating any gains to the protocol.
...
contract PoCs is Helper {
    ...
    function test_manipulateBidsListAndDumpUserOutOfIt() public {
...
Consider adding a threshold to update bids and only dump a bid if it's bigger than the lower one + the threshold. Compare the final bid value for the most recent bid to the bid that has been dumped.