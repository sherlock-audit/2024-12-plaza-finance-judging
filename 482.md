...
# `Pool.startAuction()` does not check if `Pool.distribute()` has already be called

### Summary

`Pool.distribute()` is meant to be called after a successful auction, in order to transfer collected coupons tokens from last auction to distributor contract.

A new auction can be started by anyone as along as `lastDistribution + distributionPeriod < block.timestamp <= lastDistribution + distributionPeriod + auctionPeriod`, which would increase [`bondToken.globalPool.currentPeriod`](https://github.com/sherlock-audit/2024-12-plaza-finance/blob/main/plaza-evm/src/Pool.sol#L567), however it does not check if coupons tokens from last period has already been distribute. Then, some funds can end up trapped in pool contract.

### Root Cause

https://github.com/sherlock-audit/2024-12-plaza-finance/blob/main/plaza-evm/src/Pool.sol#L530-L571
...
**MEDIUM**
Lost of yield in edge scenario. Consider that period frequency can be increase and the possible effect in case base sequencer goes down.
...
contract NoCallToDistributeCheckPOC is Test {
    Token USDC;
    Token WETH;
    ...
    function mintBondTokens(address user,uint256 wethAmount) private {
        WETH.mint(user, wethAmount);
        
        _updateOracleHeartbeat();
        vm.startPrank(user);
        WETH.approve(address(pool), wethAmount);
        pool.create(Pool.TokenType.BOND,wethAmount,1); // mint at least sth
    }
    

    function step_doAllClaims() public {
        for(uint i; i < users.length; i++){
            vm.startPrank(users[i]);
            distributor.claim();
            vm.stopPrank();
    ...
    function test_poc_NoDistributionCallCheckLeadToStuckUSDC() public {
        ...
        vm.prank(users[0]);
        preDeposit.claim();
        uint usdcBeforeClaim = USDC.balanceOf(users[0]);


        // Auction1 --> Call Pool.distribute() --> Mint new bonds --> Auction 2 --> Call Pool.distribute()
            ...
            recordAllUsersToClaimedRewards1();

            uint256 PoolUSDCBalanceAtTheEnd = USDC.balanceOf(address(pool));
            vm.revertTo(snapshot);


        // Auction1 --> Mint new bonds --> Auction 2 --> Call Pool.distribute()
        // First auction
            step_startAuction();
            step_completeAuction();


            // pool.distribute(); do not call distribute
            vm.startPrank(users[0]);
            // distributor.claim(); do not claim bc it will revert
            vm.stopPrank();
            mintBondTokens(users[0],1e18);
            ...
            pool.distribute();
            step_doAllClaims();
            recordAllUsersToClaimedRewards2();

            uint256 PoolUSDCBalanceAtTheEnd2 = USDC.balanceOf(address(pool));

            for(uint i; i < users.length; i++){
                require(usersToClaimedRewards1[users[i]] == usersToClaimedRewards2[users[i]], "User rewards mismatch");
            }

            require(PoolUSDCBalanceAtTheEnd == PoolUSDCBalanceAtTheEnd2, "Pool USDC balance mismatch");
    }
}
...
Introduce a new state variable to check if distribution for last period has already been called.

```diff
    // Pool.sol
    function startAuction() external whenNotPaused() {
        // ..
+       if(!distributionHasBeenTriggered){
+           revert DistributeHasNotBeenCalledYet();
+       }
        // ..
+       distributionHasBeenTriggered = false;
    }
    ...
    (uint256 currentPeriod,) = bondToken.globalPool();
        ...
        return;
+       }else if(Auction(auctions[previousPeriod]).state() == Auction.State.BIDDING){
+           revert AuctionIsActiveNow();
+       } 

        // ..
        Distributor(distributor).allocate(couponAmountToDistribute);
+       distributionHasBeenTriggered = true
