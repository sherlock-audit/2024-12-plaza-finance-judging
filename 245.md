...
# `Pool::transferReserveAuction` function gets inexistent auction address blocking functionality and users locking money

### Summary

The `Pool::transferReserveAuction` is called when an `Auction` is finalized. Any user can call `Auction::endAuction` to end it and this function will take care of processing state and transferring the tokens from the `Pool` to the `Auction` contract.

The problem is that the `Pool::transferReserveAuction` queries the `currentPeriod` for that `Auction` to get the `Auction` address. However, when an `Auction` is created, the `startAuction` function, using the `DISTRIBUTOR_ROLE` previously granted, calls `BondToken::increaseIndexedAssetPeriod(sharesPerToken);` Which updates the `sharesPerToken` amount and `increase` the `currentPeriod`.

This means, the `Pool::transferReserveAuction` uses accounts for a future `currentPeriod`, not the one used to store the newest created `Auction` blocking functionality and locking users' money.

### Root Cause

`Pool::transferReserveAuction` uses a future `currentPeriod` and query and inexistent `Auction` contract, blocking functionality.
...
2. It deploys an `Auction` contract
3. And then calls `BondToken::increaseIndexedAssetPeriod`, which increments the `currentPeriod` variable.
...
Common protocol flow, internal logic error.

### Impact

The `Auction` will not be terminated.
1. blocking users from refund or claim
2. Locking `Auction` to a endless `bidding` state
...
contract PoCs is Helper {
    ...
    function test_endAuctionRevertBecauseWrongAuctionAddress() public {
